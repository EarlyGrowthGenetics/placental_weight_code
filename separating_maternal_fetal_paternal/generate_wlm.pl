...
open(my $in,"zcat ".$dir."pw_fetal_sex1.tbl.gz | ") or die $!;
my %fetal=();
<$in>;
while(<$in>){
  chomp;
...
close($in);
open($in,"zcat ".$dir."pw_maternal_sex1.tbl.gz | ") or die $!;
my %maternal=();
<$in>;
while(<$in>){
  chomp;
...
close($in);
open($in,"zcat ".$dir."pw_paternal_sex1.tbl.gz | ") or die $!;
my %paternal=();
<$in>;
while(<$in>){
  chomp;
